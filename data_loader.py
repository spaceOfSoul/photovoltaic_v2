...
class WPD(Dataset): # Weather Prediction Data loader
    ...
    def __getitem__(self, idx):
        ...
        if os.path.isfile(weather_datapath):
            ...
            weather_data = np.zeros(
                [24*60, self.input_dim]
            )  # hard coding for 1 day, 7 features & time
            timeflag = np.ones(24*60)
        ...
        weather_data = torch.tensor(weather_data)
          # 일시 (시간 정보), 기온(°C), 1분 강수량(mm), 풍향(deg), 풍속(m/s), 현지기압(hPa), 해면기압(hPa), 습도(%)        
        ...
        # print(f"weather_data: {weather_data.shape}, power_data: {power_data.shape}")
        # weather_data: torch.Size([1440, 8]), power_data: torch.Size([24])
        # 1440 min = 24 hours x 60 min
